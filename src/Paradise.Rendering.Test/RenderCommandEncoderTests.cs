...
public class RenderCommandEncoderTests
{
    ...
    public async Task encoder_round_trip_preserves_command_kinds_and_payloads()
    {
        ...
        encoder.SetBindGroup(2, group, dynamicOffset: 512);
        ...
        await Assert.That(commands[4].SetBindGroup.Group).IsEqualTo(group);
        await Assert.That(commands[4].SetBindGroup.DynamicOffset).IsEqualTo(512u);
        await Assert.That(commands[4].SetBindGroup.HasDynamicOffset).IsTrue();
