...
public sealed class SamplingContext
{
    ...
    private readonly SoaVector3Keys[] _translationKeys;
    private readonly SoaQuaternionKeys[] _rotationKeys;
    private readonly SoaVector3Keys[] _scaleKeys;
    private AnimationClip? _animation;
    private float _ratio;

    public SamplingContext(int maxTracks)
    {
        MaxPaddedTracks = AnimationClip.PaddedTrackCount(Math.Max(0, maxTracks));
        var groups = MaxPaddedTracks / 4;
        _translations = new Cache(MaxPaddedTracks);
        _rotations = new Cache(MaxPaddedTracks);
        _scales = new Cache(MaxPaddedTracks);
        _translationKeys = new SoaVector3Keys[groups];
        _rotationKeys = new SoaQuaternionKeys[groups];
        _scaleKeys = new SoaVector3Keys[groups];
    }
    ...
    public void Sample(AnimationClip clip, float ratio, SoaTransforms output)
    {
        ArgumentNullException.ThrowIfNull(clip);
        ArgumentNullException.ThrowIfNull(output);
        var padded = AnimationClip.PaddedTrackCount(clip.TrackCount);
        ...
        if (!ReferenceEquals(_animation, clip))
        {
            Invalidate();
            _animation = clip;
        }
        ...
        var groups = padded / 4;
        var timepoints = clip.Timepoints.AsSpan();
        var translations = new StreamView(clip.Translations, timepoints.Length);
        var rotations = new StreamView(clip.Rotations, timepoints.Length);
        var scales = new StreamView(clip.Scales, timepoints.Length);
        var translationKeys = _translationKeys.AsSpan(0, groups);
        var rotationKeys = _rotationKeys.AsSpan(0, groups);
        var scaleKeys = _scaleKeys.AsSpan(0, groups);

        UpdateCache(clamped, previousRatio, padded, timepoints, in translations, _translations);
        DecompressVector3(groups, timepoints, in translations, _translations, translationKeys);
        UpdateCache(clamped, previousRatio, padded, timepoints, in rotations, _rotations);
        DecompressQuaternion(groups, timepoints, in rotations, _rotations, rotationKeys);
        UpdateCache(clamped, previousRatio, padded, timepoints, in scales, _scales);
        DecompressVector3(groups, timepoints, in scales, _scales, scaleKeys);
    ...
    private static void Interpolate(float ratio, int groups, ReadOnlySpan<SoaVector3Keys> translations, ReadOnlySpan<SoaQuaternionKeys> rotations, ReadOnlySpan<SoaVector3Keys> scales, SoaTransforms output)
    {
        ...
        var outT = output.Translations.AsSpan();
        var outR = output.Rotations.AsSpan();
        var outS = output.Scales.AsSpan();
    ...
    private readonly ref struct StreamView
    {
        ...
        public StreamView(KeyframeStream stream, int timepointCount)
        {
            Ratios = stream.Ratios;
            Previouses = stream.Previouses;
            Values = stream.Values;
            IframeEntries = stream.IframeEntries;
            IframeDesc = stream.IframeDesc;
    ...
    private static void UpdateCache(float ratio, float previousRatio, int paddedTracks, ReadOnlySpan<float> timepoints, in StreamView stream, Cache cache)
    {
        var numKeys = (uint)stream.KeyCount;
        var entries = cache.Entries.AsSpan(0, paddedTracks);
        var outdated = cache.Outdated.AsSpan(0, paddedTracks);
    ...
    private static void DecompressVector3(int groups, ReadOnlySpan<float> timepoints, in StreamView stream, Cache cache, Span<SoaVector3Keys> keys)
    {
        var entries = cache.Entries.AsSpan();
        var outdated = cache.Outdated.AsSpan();
    ...
    private static void DecompressQuaternion(int groups, ReadOnlySpan<float> timepoints, in StreamView stream, Cache cache, Span<SoaQuaternionKeys> keys)
    {
        var entries = cache.Entries.AsSpan();
        var outdated = cache.Outdated.AsSpan();
        for (var g = 0; g < groups; g++)
    ...
    private static void ReadQuaternions(ReadOnlySpan<ushort> values, uint k0, uint k1, uint k2, uint k3, out Vector128<float> x, out Vector128<float> y, out Vector128<float> z, out Vector128<float> w)
    {
    ...
    private sealed class Cache(int paddedTracks)
    {
        ...
        public readonly byte[] Outdated = new byte[paddedTracks];
        public uint Next;
    }
