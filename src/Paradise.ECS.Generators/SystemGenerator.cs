...
public class SystemGenerator : IIncrementalGenerator
{
    ...
    private const string SnapshotReadSystemsAttributeFullName = "Paradise.ECS.SnapshotReadSystemsAttribute";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var snapshotReadSystems = context.CompilationProvider
            .Select(static (compilation, _) =>
                compilation.Assembly.GetAttributes()
                    .Any(a => a.AttributeClass?.ToDisplayString() == SnapshotReadSystemsAttributeFullName));
    ...
    private static bool TryGetNestedViewKind(string viewName, bool isSingletonQueryable, out FieldKind kind)
    {
        ...
        {
            "ReadLookup" => FieldKind.CompositionReadLookup,
            "WriteLookup" => FieldKind.CompositionWriteLookup,
        ...
        out FieldKind resolvedKind)
    {
        ...
        if (name.EndsWith("ReadLookup", StringComparison.Ordinal) && name.Length > 10)
        {
            var prefix = name.Substring(0, name.Length - 10);
            if (queryableLookup.TryGetValue(prefix, out matched))
            {
                resolvedKind = FieldKind.CompositionReadLookup;
                return true;
            }
        }
        else if (name.EndsWith("WriteLookup", StringComparison.Ordinal) && name.Length > 11)
        {
            var prefix = name.Substring(0, name.Length - 11);
            if (queryableLookup.TryGetValue(prefix, out matched))
            {
                resolvedKind = FieldKind.CompositionWriteLookup;
                return true;
            }
        }
    ...
    private static SystemFieldInfo? AnalyzeField(IFieldSymbol field)
    {
        ...
        // arbitrary-entity access, dependency but not query filter.
        if (!isRef && fieldType is INamedTypeSymbol accessorType && accessorType.IsGenericType)
        {
            var accessorDefinition = accessorType.OriginalDefinition;
            bool isReader =
                accessorDefinition.Name == "EntityComponentReader" &&
            ...
            if ((isReader || isWriter) &&
                ...
                HasComponentOrTagAttribute(componentType))
            {
                return new SystemFieldInfo(
                    field.Name,
                    isReader ? FieldKind.EntityComponentReader : FieldKind.EntityComponentWriter,
                    isReader,
                    accessorType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat),
        ...
        bool snapshotReadSystems)
    {
        ...
        foreach (var sys in sorted)
        {
            ...
            foreach (var field in sys.Fields)
            {
                ...
                // [CurrentTick] applies only to read-only fields that can bind to either world.
                ...
                // IWorldSystem only accepts whole-query fields, arbitrary-entity accessors,
        ...
        foreach (var (info, id) in systemsWithIds)
        {
            GeneratePerSystemPartial(context, info, id, maskType, configTypeFull, snapshotReadSystems);
        }
    ...
    private static void GenerateConstructor(StringBuilder sb, SystemInfo sys, string indent, string maskType, string configType)
    {
        ...
        foreach (var field in sys.Fields)
        {
            ...
            switch (field.Kind)
            {
                ...
                case FieldKind.CompositionWriteLookup:
    ...
    private static void GenerateRunChunk(StringBuilder sb, SystemInfo sys, string indent, string maskType, string configType, bool snapshotReadSystems)
    {
        ...
        {
            if (snapshotReadSystems)
                GenerateSnapshotEntityModeRunChunk(sb, sys, indent + "    ", maskType, configType);
            else
        ...
        {
            if (snapshotReadSystems)
                GenerateSnapshotChunkModeRunChunk(sb, sys, indent + "    ", maskType, configType);
            else
    ...
    private static void GenerateSnapshotEntityModeRunChunk(StringBuilder sb, SystemInfo sys, string indent, string maskType, string configType)
    {
        ...
        if (needsBytes)
            sb.AppendLine($"{indent}var bytes = world.ChunkManager.GetBytes(chunk);");
        if (needsReadBytes)
            sb.AppendLine($"{indent}var readBytes = readChunkManager.GetBytes(readChunk);");
        foreach (var field in inlineFields)
        {
            var varName = ToCamelCase(field.FieldName) + "Span";
    ...
    private static void GenerateSnapshotChunkModeRunChunk(StringBuilder sb, SystemInfo sys, string indent, string maskType, string configType)
    {
        ...
        foreach (var field in inlineFields)
        {
            ...
            sb.AppendLine($"{indent}var {varName} = {source}.GetSpan<{compFQN}>(layout.GetBaseOffset({compFQN}.TypeId), entityCount);");
        }
    ...
    {
        var typeName = field.Kind == FieldKind.CompositionReadLookup ? "ReadLookup" : "WriteLookup";
    ...
    private static string GetEntityAccessorBinding(
        ...
        string configType)
    {
        if (field.Kind is FieldKind.CompositionReadLookup or FieldKind.CompositionWriteLookup)
        {
            var queryableSource = field.Kind == FieldKind.CompositionReadLookup && snapshotReadSystems && !field.IsCurrentTick
                ? "(readWorld ?? world)"
                : "world";
