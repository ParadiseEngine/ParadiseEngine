...
/// <summary>The engine sample's compute plasma, ported to the browser backend — the same
/// two-submit shape (compute via SubmitOffscreen, then the presenting frame) running on browser
/// WebGPU. Like the cube port, the delta is the renderer type and the loader call; everything
/// else is backend-agnostic engine code.</summary>
internal sealed class ComputeScene : IDisposable
{
    ...
    public ComputeScene(IRenderer renderer)
    {
        _renderer = renderer;

        var computeProgram = ShaderProgramLoader.Load(typeof(ComputeScene).Assembly, "Shaders.proceduralCompute");
        var displayProgram = ShaderProgramLoader.Load(typeof(ComputeScene).Assembly, "Shaders.computeDisplay");
