...
{
    /// <summary>Row-vector convention: <c>model[i] = local[i] × model[parent]</c>, roots multiplied by <paramref name="root"/> (identity when null). The four local matrices of a group are built in lanes, then each multiplies its parent.</summary>
    /// <exception cref="ArgumentException">Fewer poses or outputs than joints.</exception>
    public static void Compute(ref SkeletonBlob skeleton, ref JointPoses locals, Span<Matrix4x4> models, in Matrix4x4? root = null)
    {
        ...
        var translations = locals.Translations.ToSpan();
        var rotations = locals.Rotations.ToSpan();
        var scales = locals.Scales.ToSpan();
        ...
        for (var g = 0; g * 4 < count; g++)
        {
            ...
            for (var lane = 0; lane < lanes; lane++)
            {
                ...
                models[joint] = local * (parent == SkeletonBlob.NoParent ? rootMatrix : models[parent]);
            }
        }
    }
