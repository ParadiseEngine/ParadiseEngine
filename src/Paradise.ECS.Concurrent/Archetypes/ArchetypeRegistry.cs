...
public sealed class ArchetypeRegistry<TMask, TConfig>
    : IArchetypeRegistry<TMask, TConfig, Archetype<TMask, TConfig>>, IDisposable
    ...
    where TConfig : IConfig, new()
{
    ...
    private readonly ConcurrentAppendOnlyList<Archetype<TMask, TConfig>?> _archetypes = new();
    private readonly ConcurrentAppendOnlyList<List<Archetype<TMask, TConfig>>?> _queryCache = new();
    ...
    public ArchetypeRegistry(SharedArchetypeMetadata<TMask, TConfig> sharedMetadata, ImmutableArray<ComponentTypeInfo> typeInfos, ChunkManager chunkManager)
    {
        ArgumentNullException.ThrowIfNull(sharedMetadata);
        if (typeInfos.IsDefault)
    ...
    public bool TryGet(HashedKey<TMask> mask, out Archetype<TMask, TConfig>? store)
    {
        ThrowHelper.ThrowIfDisposed(_disposed != 0, this);
        using var _ = _operationGuard.EnterScope();
