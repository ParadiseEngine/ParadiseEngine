...
/// <summary>
/// Entry point for the asset project's Coyote tests. Run with: <c>dotnet run [iterations]</c>.
///
...
public static class Program
{
    public static int Main(string[] args)
    {
        ...
        Console.WriteLine($"Running Coyote asset-project tests with {iterations} iterations...");
        Console.WriteLine();

        var tests = new (string Name, Func<Task> Action)[]
        {
            ("ConcurrentFirstUse_PreparesOnceAndStaysEnabled", ArtifactCacheTests.ConcurrentFirstUse_PreparesOnceAndStaysEnabled),
            ("TwoStoresOfOneKey_LandOneWholeEntry", ArtifactCacheTests.TwoStoresOfOneKey_LandOneWholeEntry),
            ("StoreRacingFetch_NeverServesAPartial", ArtifactCacheTests.StoreRacingFetch_NeverServesAPartial),
        };
        ...
        foreach (var (name, action) in tests)
        {
            var configuration = Microsoft.Coyote.Configuration.Create()
                ...
                // These tests AWAIT their joins rather than blocking on them, so hang detection
                // stays ON and is a signal here.
                ;
